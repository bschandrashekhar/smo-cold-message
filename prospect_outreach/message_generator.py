...
Para 2: Pick up the GENERAL-PURPOSE technology names from "Prospect_Technologies" (e.g. Boomi, Snowflake, MuleSoft) — NOT proprietary systems. Weave them into a case study narrative based ONLY on facts from the case studies provided above — do NOT invent, fabricate, or embellish details beyond what the case studies state. If the prospect's exact technology (e.g. Dynamics) does not appear in any case study, do NOT fabricate a story about it. Instead, bridge lightly: acknowledge the prospect's technology ecosystem and show how we have done Salesforce work alongside similar technologies (e.g. Azure, .NET) that DO appear in the case studies. If "Prospect_Technologies" mentions Boomi, Para 2 references how we've helped clients with Boomi integration leveraging Salesforce. Anonymize as "a leading [industry] company". Only reference these approved Salesforce products (and ONLY if clearly relevant): {approved_sf_products}. If no specific product fits, just say "Salesforce".
